...
export const Checkout: React.FC<Props> = ({ onBack, onConfirmOrder }) => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          setErrorMessage('No se pudieron convertir las coordenadas a una dirección válida.');
        } finally {
          setLoadingGps(false);
        }
      },
      (error) => {
        console.error(error);
        setLoadingGps(false);
        setErrorMessage('No se pudo acceder a tu ubicación GPS. Por favor introdúcela manualmente.');
      },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (paymentMethod === 'pago_movil' && !/^\d{4}$/.test(comprobante)) {
      setErrorMessage('Ingresa exactamente los últimos 4 dígitos del comprobante de Pago Móvil.');
      return;
    }
    ...
    try {
      ...
      if (orderData && orderData.id) {
        ...
        if (!existingOrders.includes(orderData.id)) {
          existingOrders.push(orderData.id);
          localStorage.setItem('my_orders', JSON.stringify(existingOrders));
        }
      }

      // Vaciar el carrito en el store
    ...
    <div className="min-h-screen bg-[#0d0c0a] text-neutral-100 flex flex-col justify-between font-sans">
      <div>
        ...
        <main className="max-w-7xl mx-auto px-6 py-10">
          {/* Banner de error */}
          {errorMessage && (
            <div className="mb-6 bg-red-950/80 border border-red-500/50 rounded-xl p-4 flex items-center gap-3 text-red-200 text-sm">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Ej. Apt 4B, Edif. Central"
                        className="w-full bg-[#1e1b15] border border-[#332b1c] rounded-xl px-4 py-3 text-sm text-white placeholder-neutral-600 focus:outline-none focus:border-amber-400 transition-colors"
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Dejar en garita, llamar al llegar..."
              ...
              <div className="bg-[#14120e] border border-[#2a2419] rounded-2xl p-6 shadow-xl sticky top-24">
                ...
                <div className="flex flex-col gap-3 mb-6">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-white font-bold text-sm">Efectivo contra entrega</span>
                    </div>
                    <span className="text-neutral-400 text-xs">💵</span>
                  </div>
                </div>
                  ...
                  <div className="bg-[#181510] border border-[#2b2418] rounded-xl p-4 mb-6">
                    ...
                    <div className="flex flex-col gap-2.5 text-xs">
                      <div className="flex justify-between items-center bg-[#0e0d0a] p-2.5 rounded-lg border border-[#241e14]">
                        <div>
                          <span className="text-neutral-500 block text-[10px]">Banco</span>
                      ...
                      <div className="flex justify-between items-center bg-[#0e0d0a] p-2.5 rounded-lg border border-[#241e14]">
                        <div>
                          <span className="text-neutral-500 block text-[10px]">Teléfono</span>
                ...
                {paymentMethod === 'zelle' && (
                  <div className="bg-[#181510] border border-[#2b2418] rounded-xl p-4 mb-6">
                    ...
                    <div className="flex justify-between items-center bg-[#0e0d0a] p-2.5 rounded-lg border border-[#241e14]">
                      <div>
                        <span className="text-neutral-500 block text-[10px]">Correo Electrónico</span>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full mt-6 bg-amber-400 hover:bg-amber-500 disabled:opacity-50 text-neutral-950 font-black py-4 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg shadow-amber-500/10 text-base cursor-pointer"
                >
                  {submitting ? (
                    <>
      ...
      <footer className="bg-[#080706] border-t border-[#1f1a12] py-6 px-6 text-neutral-500 text-xs">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <p>© 2026 PanaDrink Delivery. Licorería 24/7.</p>
